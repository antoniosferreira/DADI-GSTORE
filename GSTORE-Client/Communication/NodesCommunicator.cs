...
namespace GSTORE_Client.Communication
{
    class NodesCommunicator
    {
        ...
        public string GetServerIDByClient(StorageServerServices.StorageServerServicesClient server)
        {
            return Servers.Where(x => x.Value.Equals(server) && x.Value.Item1).Select(t => t.Key).First();
        }
