            ...
            foreach (StorageServerServices.StorageServerServicesClient server in serversList)
            {
                void requestServer()
                {
                    try
                    {
                        ...
                        listings.Add("--------------- SERVER" + Client.NodesCommunicator.GetServerIDByClient(server));
                    ...
                    {
                        Console.WriteLine(">>> Server %s failed", Client.NodesCommunicator.GetServerIDByClient(server));
